...
public class Utils {
    public static int maxCountWarp(@NotNull ServerPlayer player) {
        return PermissionUtil.maxValue(ExtraWarp.getInstance().getServerConfig().getBaseCountWarp(), player, PermissionNodes.PERMISSION_LIST_MODIFY);
    }
    ...
    public static boolean isBlockedNameWarp(@NotNull String warpName) {
        val blockedNamesWarp = Sets.newHashSet("set", "pset", "delete", "private", "invite", "uninvite",
                "blacklist", "public", "rename", "welcome", "top", "help", "reload", "assets", "info", "update");

        blockedNamesWarp.addAll(ExtraWarp.getInstance().getServerConfig().getBlockedNamesWarp());

        return blockedNamesWarp.contains(warpName.toLowerCase(Locale.ROOT));
    }
}